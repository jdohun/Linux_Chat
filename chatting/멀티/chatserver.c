...
struct client_node* add_client(int cfd) {		// main에서 사용
...
*/
struct client_node* search_client_list(char* recipient) {		// handle_client에서 사용
	struct client_node* p = client_list;
...
void remove_client(struct client_node* c) {		// handle_client에서 사용
...
char* get_username(struct client_node* cnode) {		// handle_client에서 사용
	char* str = malloc(38);
...
void* handle_client(void* c) { 		// main에서 사용
