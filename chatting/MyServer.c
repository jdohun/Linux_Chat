...
void welcome(void* c) {
	struct client_node* cnode, * tmpnode;
	char* formatted_msg;

	cnode = (struct client_node*)c;
	// 이름을 저장한 cnode를 매개변수로 받았기 때문에 이름 저장을 다시 하지 않는다.
	//strcpy(cnode->username, get_username(cnode));
	
	tmpnode = client_list;
	formatted_msg = malloc(BUFF_SIZE);

	sprintf(formatted_msg, "[%s] %s", cnode->username, "enterd!");
	printf("%s\n", formatted_msg);
	while (tmpnode) {
	...
	while (1) {
		...
		*/
		...
		if (strncmp(buffer, "all ", 4) == 0) {
			/* 버퍼를 msg 와 recipient로 분리*/
			tmp = strchr(buffer, ' ');
			if (tmp == NULL)
				continue;
			msg = tmp + 1;

			/* list 안에 있는 모든 클라이언트를 부른다.*/
			tmpnode = client_list;

			while (tmpnode) {
				if (strcmp(cnode->username, tmpnode->username) == 0) {
					tmpnode = tmpnode->next;
					continue;
				}
				/* on invalid recipient, do nothing */
				...
				sprintf(formatted_msg, "%s: %s", cnode->username, msg);
				/* logging in the server */
				printf("%s sent msg to %s\n", cnode->username, tmpnode->username);
				/* Hey target client, You've got message ;) */
				write(tmpnode->sockfd, formatted_msg, strlen(formatted_msg) + 1);
				tmpnode = tmpnode->next;
			}
